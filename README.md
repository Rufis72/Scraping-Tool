...
```shell
python3 -m mangadl download 'https://bato.to/title/83510-one-piece-official'
```
Download a manga via searching
```shell
python3 -m mangadl download -s 'One Piece'
```
Download a manga via searching on a specific website
```shell
python3 -m mangadl download -s 'One Piece' -w bato
```
List all possible websites to search on specifically
```shell
python3 -m mangadl download --list-ids
```
Download a specific chapter of a manga
```shell
python3 -m mangadl download 'https://bato.to/title/83510-one-piece-official' -c 1146
```
Download multiple chapters
```shell
python3 -m mangadl download -s 'One Piece' -c 1100:1146
```
Format downloaded manga as a PDF (formatting it as one image per page)
```shell
python3 -m mangadl format -i ./One\ Piece -o ./output.pdf
```
Format downloaded webtoons as a PDF (formatting it as one chapter per page, so every new image is placed below the old one)
```shell
python3 -m mangadl format -i ./Hero\ Killer -o ./output.pdf --content-format webtoon
```
Format a series as a PDF, but split it up into multiple files by chapter count
```shell
mkdir output
python3 -m mangadl format -i ./One\ Piece -o ./output --chapters-per-file 20 --file-format pdf
```
